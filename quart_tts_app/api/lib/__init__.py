...
import onnxruntime
import phonemes2ids
from quart_tts_app.api.lib.glow_tts import GlowTextToSpeech

from quart_tts_app.api.lib.audio import AudioSettings
...
from quart_tts_app.api.lib.utils import (
    VOCODER_QUALITY,
    get_voices_dirs,
    resolve_voice_name,
    split_voice_name,
...
def load_vocoder_model(
    ...
    if model_type == VocoderType.GRIFFIN_LIM:
        from quart_tts_app.api.lib.griffin_lim import GriffinLimVocoder

        return GriffinLimVocoder(config)

    if model_type == VocoderType.HIFI_GAN:
        from quart_tts_app.api.lib.hifi_gan import HiFiGanVocoder
